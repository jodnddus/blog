...
이를 위한 대안으로 구글 폼, 서베이 몽키, 타입폼과같은 웹 기반 설문 진행 애플리케이션을 만들었는데요.

이 글에서는 포켓서베이 웹 설문에만 존재하는 기능들을 어떻게 개발하였는지 코드와 함께 살펴보려고 합니다.

# 포켓서베이 웹 설문의 특별한 기능들과 그 기능들이 만들어진 이유

포켓서베이의 웹 설문에만 존재하는 기능은 아래와 같습니다.

1. 브랜칭을 고려해서 문항을 표시하는 기능
2. 문항 포커싱 기능 만들기

그렇다면 이 기능들을 왜 만들었을까요?

## 브랜칭을 고려해서 문항을 표시하기

포켓서베이의 설문 제작 페이지에는 응답자의 응답에 따라서 다른 문항을 나오게 하는 `브랜칭` 기능이 존재합니다. 이 브랜칭이 적용되어 있는 설문에 응답을 할 때는 응답 흐름에 맞는 문항들이 표시되어야 하는데요, 이 브랜칭 기능을 고려하여 문항을 표시하기 위해 약간의 고민이 필요했습니다.
...
만약 문항을 표시를 후자의 방법으로 했다면 이 기능을 넣을 필요는 없었을 것입니다. 하지만 여러 문항을 한 번에 표시함으로 인해 사용자가 지금 어떤 문항을 보고 있는지 표시해 줄 필요가 생겼습니다.
...
```json
{
  "showing": [true, true, true, false, false, false, true, false, false]
}
```

이런 모양의 showing 배열이 있다면, 총 1,2,3까지 응답하고 3번에서 어떤 응답을 했는데 7번으로 곧바로 넘어간 상황이 된겁니다.
